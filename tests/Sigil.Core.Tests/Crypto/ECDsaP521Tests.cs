...
public class ECDsaP521Tests
{
    [Fact]
    public void Generate_ProducesValidSigner()
    {
        using var signer = ECDsaP521Signer.Generate();
    ...
    public void SignAndVerify_RoundTrip_Succeeds()
    {
        ...
        var verifier = ECDsaP521Verifier.FromPublicKey(signer.PublicKey);
    ...
    {
        using var signer1 = ECDsaP521Signer.Generate();
        using var signer2 = ECDsaP521Signer.Generate();
        var data = Encoding.UTF8.GetBytes("message signed by key 1");
        var signature = signer1.Sign(data);

        var verifierWithKey2 = ECDsaP521Verifier.FromPublicKey(signer2.PublicKey);
    ...
    public void ExportImport_Pkcs8_RoundTrip()
    {
        ...
        using var restored = ECDsaP521Signer.FromPkcs8(pkcs8);

        var verifier = ECDsaP521Verifier.FromPublicKey(original.PublicKey);
    ...
    public void ExportImport_EncryptedPkcs8_RoundTrip()
    {
        ...
        using var restored = ECDsaP521Signer.FromEncryptedPkcs8(encrypted, password);
    ...
    public void ExportImport_Pem_RoundTrip()
    {
        ...
        var verifier = ECDsaP521Verifier.FromPublicKeyPem(pem);
    ...
    public void ExportImport_PrivateKeyPem_RoundTrip()
    {
        ...
        using var restored = ECDsaP521Signer.FromPem(pem.AsSpan());
    ...
    {
        var signer = ECDsaP521Signer.Generate();
        signer.Dispose();
