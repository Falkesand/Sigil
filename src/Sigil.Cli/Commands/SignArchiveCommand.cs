...
public static class SignArchiveCommand
{
    public static Command Create()
    {
        var archiveArg = new Argument<string>("archive") { Description = "Path to the archive file (ZIP, tar.gz, tar, nupkg)" };
        ...
        var cmd = new Command("sign-archive", "Sign an archive file with per-entry digests");
        cmd.Add(archiveArg);
        cmd.Add(keyOption);
        ...
        cmd.SetAction(async parseResult =>
        {
            ...
            if (format is null)
            {
                Console.Error.WriteLine($"Unrecognized archive format: {archivePath}");
                return;
            }
            ...
            var outputPath = output ?? archivePath + ".archive.sig.json";
            ...
            if (vaultName is not null)
            {
                ...
                var envelope = await SignOrAppendAsync(archivePath, format.Value, outputPath, signer, fingerprint, label);
            ...
            if (certStoreThumbprint is not null)
            {
                ...
                var envelope = await SignOrAppendAsync(archivePath, format.Value, outputPath, certSigner, fingerprint, label);
                await ApplyTimestampIfRequestedAsync(envelope, tsaUrl);
                await SubmitToLogIfRequestedAsync(envelope, logUrl, logApiKey);
            ...
            using (localSigner)
            {
                ...
                var envelope = SignOrAppend(archivePath, format.Value, outputPath, localSigner, fingerprint, label);
                await ApplyTimestampIfRequestedAsync(envelope, tsaUrl);
                await SubmitToLogIfRequestedAsync(envelope, logUrl, logApiKey);
    ...
    private static ManifestEnvelope SignOrAppend(
        string archivePath, ArchiveFormat format, string outputPath,
        ISigner signer, KeyFingerprint fingerprint, string? label)
    {
        if (File.Exists(outputPath))
        {
            var existingJson = File.ReadAllText(outputPath);
            var envelope = ArchiveSigner.Deserialize(existingJson);
            ArchiveSigner.AppendSignature(envelope, signer, fingerprint, label);
            return envelope;
        }

        return ArchiveSigner.Sign(archivePath, format, signer, fingerprint, label);
    }
        ...
        ISigner signer, KeyFingerprint fingerprint, string? label)
    {
        if (File.Exists(outputPath))
        {
            ...
            var envelope = ArchiveSigner.Deserialize(existingJson);
            await ArchiveSigner.AppendSignatureAsync(envelope, signer, fingerprint, label).ConfigureAwait(false);
            return envelope;
        }

        return await ArchiveSigner.SignAsync(archivePath, format, signer, fingerprint, label).ConfigureAwait(false);
    }
    ...
    private static async Task ApplyTimestampIfRequestedAsync(ManifestEnvelope envelope, string? tsaUrl)
    {
        ...
        {
            Console.Error.WriteLine($"Warning: Invalid TSA URL: {tsaUrl}. Archive signed without timestamp.");
            return;
        }
        ...
        {
            Console.Error.WriteLine($"Warning: Timestamping failed ({result.ErrorMessage}). Archive signed without timestamp.");
        }
    }
    ...
    {
        Console.WriteLine($"Archive signed: {envelope.Subjects.Count} entries");
        ...
        ManifestEnvelope envelope, string? logUrl, string? logApiKey)
    {
        ...
        {
            Console.Error.WriteLine($"Warning: {ex.Message} Archive signed without log entry.");
            return;
        }
        ...
        using (remoteLog)
        {
            ...
            {
                Console.Error.WriteLine($"Warning: Log submission failed ({result.ErrorMessage}). Archive signed without log entry.");
            }
        }
    }
}
