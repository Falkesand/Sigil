...
public sealed class SqlServerLogStore : ILogStore
{
    ...
    public async Task InitializeAsync(CancellationToken ct = default)
    {
        ...
        cmd.CommandText = """
            IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'log_entries')
            CREATE TABLE log_entries (
                id BIGINT IDENTITY(1,1) PRIMARY KEY,
                timestamp NVARCHAR(100) NOT NULL,
                key_id NVARCHAR(200) NOT NULL,
                algorithm NVARCHAR(50) NOT NULL,
                artifact_name NVARCHAR(500) NOT NULL,
                artifact_digest NVARCHAR(200) NOT NULL,
                signature_digest NVARCHAR(200) NOT NULL UNIQUE,
                label NVARCHAR(200),
                leaf_hash NVARCHAR(200) NOT NULL,
                raw_json NVARCHAR(MAX) NOT NULL
            );

            IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_log_entries_key_id')
            CREATE INDEX idx_log_entries_key_id ON log_entries(key_id);

            IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_log_entries_artifact_name')
            CREATE INDEX idx_log_entries_artifact_name ON log_entries(artifact_name);

            IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_log_entries_artifact_digest')
            CREATE INDEX idx_log_entries_artifact_digest ON log_entries(artifact_digest);

            IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_log_entries_signature_digest')
            CREATE INDEX idx_log_entries_signature_digest ON log_entries(signature_digest);

            IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'checkpoints')
            CREATE TABLE checkpoints (
                id BIGINT IDENTITY(1,1) PRIMARY KEY,
                tree_size BIGINT NOT NULL,
                root_hash NVARCHAR(200) NOT NULL,
                timestamp NVARCHAR(100) NOT NULL,
    ...
    public async Task<long> AppendEntryAsync(LogStoreEntry entry, CancellationToken ct = default)
    {
        ...
        cmd.CommandText = """
            ...
            OUTPUT INSERTED.id
            VALUES (@timestamp, @keyId, @algorithm, @artifactName, @artifactDigest, @signatureDigest, @label, @leafHash, @rawJson);
            """;
    ...
    public async Task<IReadOnlyList<LogStoreEntry>> ListEntriesAsync(int limit, int offset, CancellationToken ct = default)
    {
        await using var connection = await CreateConnectionAsync(ct).ConfigureAwait(false);
        await using var cmd = connection.CreateCommand();
        cmd.CommandText = "SELECT id, timestamp, key_id, algorithm, artifact_name, artifact_digest, signature_digest, label, leaf_hash, raw_json FROM log_entries ORDER BY id OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY;";
    ...
    public async Task<IReadOnlyList<LogStoreEntry>> SearchAsync(LogSearchQuery query, CancellationToken ct = default)
    {
        ...
        cmd.CommandText = $"SELECT id, timestamp, key_id, algorithm, artifact_name, artifact_digest, signature_digest, label, leaf_hash, raw_json FROM log_entries {where} ORDER BY id OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY;";
    ...
    public async Task<LogStoreCheckpoint?> GetLatestCheckpointAsync(CancellationToken ct = default)
    {
        await using var connection = await CreateConnectionAsync(ct).ConfigureAwait(false);
        await using var cmd = connection.CreateCommand();
        cmd.CommandText = "SELECT TOP 1 id, tree_size, root_hash, timestamp, signature FROM checkpoints ORDER BY id DESC;";
    ...
    private static LogStoreEntry ReadEntry(SqlDataReader reader)
    {
    ...
    private static async Task<List<LogStoreEntry>> ReadEntriesAsync(SqlCommand cmd, CancellationToken ct)
    {
        await using var reader = await cmd.ExecuteReaderAsync(ct).ConfigureAwait(false);
