...
public sealed class SqliteLogStore : ILogStore
{
    ...
    private static LogStoreEntry ReadEntry(SqliteDataReader reader)
    {
    ...
    private static async Task<List<LogStoreEntry>> ReadEntriesAsync(SqliteCommand cmd, CancellationToken ct)
    {
        await using var reader = await cmd.ExecuteReaderAsync(ct).ConfigureAwait(false);
