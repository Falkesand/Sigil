...
public readonly record struct GitResult<T>
{
    ...
    private readonly GitErrorKind? _errorKind;
    ...
    public GitErrorKind ErrorKind => !IsSuccess
        ? _errorKind!.Value
    ...
    private GitResult(T? value, GitErrorKind? errorKind, string? errorMessage, bool isSuccess)
    {
        _value = value;
    ...
    public static GitResult<T> Ok(T value) => new(value, null, null, true);

    public static GitResult<T> Fail(GitErrorKind errorKind, string errorMessage) =>
